...
func Login(user,passwd,api_token,urlhost,protocol string,skipSSL bool) (bool,error) {
	...
	if resp.StatusCode != 200 {
		return false,errors.New("connect xui err, Urlpath or password err")
	}
	...
	return false,errors.New("connect xui err, user/passwd err")
...
func AddData(class ClassData,urlhost,protocol string,group int,skipSSL bool) error {
		...
		Clients: []Client{
			{
				...
				Email:      "xboard_"+class.UUID+"_"+strconv.Itoa(group),
				LimitIP:    0,
				TotalGB:    0,
				ExpiryTime: 0,
				Enable:     true,
	...
	apiaddr:=urlhost+"/xui/inbound/addClient"
...
func DelData(class ClassData,urlhost,protocol string,group int,skipSSL bool) error {
    ...
    if protocol == "shadowsocks" {
		userinfo="xboard_"+class.UUID+"_"+strconv.Itoa(group)
	}else{
		userinfo=class.UUID
	}
	
	apiaddr:=urlhost+"/xui/inbound/"+strconv.Itoa(group)+"/delClient/"+userinfo 
...
func ListData(urlhost,protocol string,group int,skipSSL bool) ([]ClassData,error) {
    apiaddr:=urlhost+"/xui/inbound/list"
	...
	for _, obj := range jsonResp.Obj {
        if obj.ID == group {
           for _, client := range obj.ClientStats {
			client.UUID=strings.TrimPrefix(strings.TrimSuffix(client.Email,"_"+strconv.Itoa(group)),"xboard_")
...
func ChangeData(class ClassData,urlhost,protocol string,group int,skipSSL bool) error {
    ...
    apiaddr := urlhost + "/xui/inbound/updateClient/" + clientID
